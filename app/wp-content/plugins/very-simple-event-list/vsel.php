...
function vsel_taxonomy() { 
	register_taxonomy( 'event_cat', 'event', array( 'label' => __( 'Event Categories', 'very-simple-event-list' ), 'hierarchical' => true, 'show_in_rest' => true, ) ); 
}
...
function vsel_metabox_callback( $post ) { 
	...
	$event_end_date = get_post_meta( $post->ID, 'event-date', true );
	...
	<input class="widefat" id="vsel-date" type="text" name="vsel-date" required maxlength="10" placeholder="<?php _e( 'Use datepicker', 'very-simple-event-list' ); ?>" value="<?php echo date_i18n( $dateformat, esc_attr( $event_end_date ) ); ?>" /></p>
...
// get event categories and create event css class
